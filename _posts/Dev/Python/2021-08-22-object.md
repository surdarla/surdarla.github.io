...
description: >
  객체, 클래스, 상속
image: 
  path: /assets/img/blog/python-object/semiotic_triangle.png
...
그래 무슨 말인지 읽히기는 하는데, 뭔말인지 알 수가 없다. 우리 일상 속에서 말하는 객체라는 것은 (주체란 말의 대칭으로써) 그저 물건들을 이야기하는 뉘앙스다.

![기호 삼각형][semiotic triangle.png]

언어학의 기호삼각형을 들여다 보면 referent가 우리가 공부하고 있는 객체이다. 우리가 '개'라고 부르는 친구를
...
이런 삼각형의 꼭지점들이 서로 이래저래 주고받으면서 우리는 의미를 효율적으로 찾을 수 있고, 사용하게 된다.

여기서 우리가 보는 실제 '개'가 우리가 프로그래밍에서 찾는 객체라는 친구이다. 우리가 개념적으로 생각하는 개의 특징을 실제로 가지고 있는 현실의 강아지 한 마리가 객체이다. 프로그램 안에서는 우리가 생각하는 속성과 기능을 가진 구현의 대상을 객체라고 볼 수 있을 것이다.
...
... 이런식으로 인풋해줄 수는 있겠으나, 많은 데이터가 쌓인다면 관리하기가 엄청 힘들 것이다.

그래서 강아지(객체)의 여러 속성을 포괄하는 개념틀(클레스class)을 만들고 각 강아지들(인스턴스) 마다 그 속성들의 묶음으로 관리하는 것! 이정도로 비유할 수 있겠다.
...
이렇게 클래스는 구현을 했다. 나의 몸이라는 객체가 들어갈 공간을 만들고, 속성인 몸무게와 키가 들어갈 공간을 마련하고, 그 들어간 속성들이 bmi라는 메서드에서 어우러져서 우리가 원하던 답이 나올 것이다. 여기서 좀 특이한 생성자라는 함수가 있다.
...
보끄랍지만 나의 개인정보를 사용해보았다.

삐빅.. 26.23 비만입니다... 알고있엇지만 다시 친절하게 알려주는 우리의 친절한 컴퓨터 친구다.
...
여기서 나 = Malja = object이고, body라는 class에 객체를 집어넣고, 객체의 속성들을 간편하게 나열해서 넣어줄 수 있다. 이렇게 객체를 만들어놓고 함수를 붙이기만 하면 어떤 것이든 할 수 있다.

생성자로 속성들을 '나'라는 인스턴스에 달아놨으니, 나의 속성을 파악하는 것은 이제 쉬워진다. 인스턴스에 점 하나 붙이고 알고 싶은 속성을 적으면 된다.
...
그리고 부모클래서 body에 새로운 매서드인 성별로 차이가 있는 권장몸무게를 만들어봤다.

이제 남자말고 여자인 Malsuni 라는 객체를 만들어보자.  말순이는 나와 같이 엄청난 피지컬의 소유자다. 그리고 body클래스를 받던 Malja를 Male클라스로 받아보자.
...
[semiotic_triangle]:/assets/img/blog/python-object/semiotic_triangle.png
