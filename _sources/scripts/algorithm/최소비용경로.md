...
```{figure} https://www.google.com/url?sa=i&url=http%3A%2F%2Fusers.cecs.anu.edu.au%2F~Alistair.Rendell%2FTeaching%2Fapac_comp3600%2Fmodule2%2Fbinary_heaps.xhtml&psig=AOvVaw00Rg2ZGCQKDjISdRG_sMTI&ust=1694761123929000&source=images&cd=vfe&opi=89978449&ved=0CBAQjRxqFwoTCKii_afDqYEDFQAAAAAdAAAAABAQ
heap에 대한 설명
```
...
stable sort
정렬이 안정적이라는 것은, 입력 데이터 내에서 동일한 값들 사이에 원래 순서가 정렬 후에도 보존된다는 것을 말한다.
...
sorted는 2002년 Tim Peters에 의해 개발된 merge sort + insertion sort 식의 하이브리드 정렬 알고리즘으로 1) 이미 부분정렬에 대해서는 더 빠르게 동작하고 2) 같은 키값의 정렬에 대해서는 순서를 보존하는 3) O(n logn)(최악에도)의 시간복잡도를 가지는 정렬이다.
